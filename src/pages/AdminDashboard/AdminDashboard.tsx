...
export const AdminDashboard: React.FC = () => {
  ...
  const [cajasAMostrar, setCajasAMostrar] = useState<any[]>([]);
  ...
  const [distContribuyentesEstado, setDistContribuyentesEstado] = useState<any[]>([]);
  const [distContribuyentesSede, setDistContribuyentesSede] = useState<any[]>([]);
  const [distContribuyentesTerminal, setDistContribuyentesTerminal] = useState<any[]>([]);
  const [distContribuyentesEstadoSede, setDistContribuyentesEstadoSede] = useState<any[]>([]);
    ...
    const fetchAdvancedData = async () => {
      try {
        ...
        const [
          ...
          dashboardService.getPrincipal(user?.token || ''),
          dashboardService.getMensual(user?.token || ''),
          dashboardService.getRankingEmpresas(user?.token || ''),
          dashboardService.getRankingUsuarios(user?.token || ''),
          dashboardService.getAlertasCajas(user?.token || ''),
          movimientoService.getMovimientos(user?.token || ''),
          CajaService.getCajas(user?.token || ''),
          sedeService.getAll(user?.token || ''),
          dashboardService.getEmpresasDetalle(user?.token || ''),
          contribuyenteService.getAll(user?.token || '').catch(() => [])
        ]);

        setPrincipal(p);
        setMensual(m);
        setEmpresas(e.slice(0, 3));
        setUsuarios(u.slice(0, 3));
        setAlertas(a);
        ...
        const sedesData = sedesListResponse || [];
        setSedesMaster(sedesData);

        // Parse Empresas Dist
        const eDist = (empresasDetalle || []).map((emp: any, idx: number) => ({
          name: emp.razon_social || `Empresa ${idx+1}`,
          value: Number(emp.total_saldo) || 0,
          fill: emp.color || COLORS_EMPRESAS[idx % COLORS_EMPRESAS.length]
        })).filter((e: any) => e.value > 0);
        setDistEmpresas(eDist);

        // Parse Contribuyentes Dist
        const cList = contribuyentesList || [];
        let nat = 0, jur = 0;
        let act = 0, inact = 0;
        const sedesCounts: Record<string, number> = {};
        const terminalCounts: Record<string, number> = {};
        const estadoSedeCounts: Record<string, { activos: number; inactivos: number }> = {};

        cList.forEach((c: any) => {
          if (c.tipo_ruc === 1 || (c.tipo_ruc == null && c.ruc && c.ruc.startsWith('20'))) jur++; else nat++;
          if (c.estado === 1) act++; else inact++;
          
          const sName = c.sede_nombre || c.sede || 'Sin Sede';
          sedesCounts[sName] = (sedesCounts[sName] || 0) + 1;
          
          if (!estadoSedeCounts[sName]) estadoSedeCounts[sName] = { activos: 0, inactivos: 0 };
          if (c.estado === 1) estadoSedeCounts[sName].activos++;
          else estadoSedeCounts[sName].inactivos++;
        ...
        if (act > 0 || inact > 0) {
          setDistContribuyentesEstado([
            ...(act > 0 ? [{ name: 'Activos', value: act, fill: '#10b981' }] : []),
            ...(inact > 0 ? [{ name: 'Inactivos', value: inact, fill: '#ef4444' }] : [])
          ]);
        }
        
        const sDistArr = Object.keys(sedesCounts).map((k, idx) => ({
          name: k,
          value: sedesCounts[k],
          fill: COLORS_SEDES[idx % COLORS_SEDES.length]
        })).filter(s => s.value > 0);
        setDistContribuyentesSede(sDistArr);
        ...
        const esDistArr = Object.keys(estadoSedeCounts).map(k => ({
          name: k,
          Activos: estadoSedeCounts[k].activos,
          Inactivos: estadoSedeCounts[k].inactivos
        }));
        setDistContribuyentesEstadoSede(esDistArr);
        ...
        const sDist = sedesConMovimientos.map((s: any, idx: number) => {
          return {
            name: s.sede,
            value: Number(s.total_ingresos) - Number(s.total_egresos),
            fill: s.color || COLORS_SEDES[idx % COLORS_SEDES.length]
          };
  ...
  useEffect(() => {
    if (estadoCajaSede === '') {
      setCajasAMostrar(cajasMaster);
    } else {
      const fetchSedeCajas = async () => {
        const res = await CajaService.getCajasSedeSaldosById(user?.token || '', Number(estadoCajaSede));
        if (res.success && res.data) {
          setCajasAMostrar(res.data);
        } else {
          setCajasAMostrar([]);
        }
      };
  ...
  }, [estadoCajaSede, cajasMaster, user?.token]);
  ...
  const handleExportExcel = async (isDaily = false) => {
    ...
    try {
      ...
      const sheetName = isDaily ? 'Caja Diaria' : 'Caja General';
      const sheet = workbook.addWorksheet(sheetName, { views: [{ showGridLines: false }] });

      let dataToExport = allTransacciones;
      if (isDaily) {
        dataToExport = allTransacciones.filter(tx => {
          const txDate = tx.fecha?.split('T')[0] || tx.updated_at?.split('T')[0] || '';
          const selectedDateStr = selectedDate.toISOString().split('T')[0];
          return txDate.startsWith(selectedDateStr) || txDate === selectedDateStr;
        });
      }
      ...
      const cajasUnicas = Array.from(new Set(dataToExport.map(t => t.caja).filter(Boolean)));
      ...
      const sortedTxs = [...dataToExport].sort((a, b) => new Date(a.fecha || 0).getTime() - new Date(b.fecha || 0).getTime());
      ...
      link.setAttribute('download', `Reporte_${isDaily ? 'DIARIO' : 'GENERAL'}_Caja.xlsx`);
  ...
  const handleExportPDF = async (isDaily = false) => {
    ...
    try {
      ...
      doc.text(isDaily ? 'Reporte Diario' : 'Consolidado Mensual de Sede', pageWidth / 2, 145, { align: 'center' });
      ...
      if (isDaily) {
        ...
        dataToExport = allTransacciones.filter(tx => {
          const txDate = tx.fecha?.split('T')[0] || tx.updated_at?.split('T')[0] || '';
          const selectedDateMonth = selectedDate.toISOString().substring(0, 7);
          return txDate.startsWith(selectedDateMonth);
        });
      }

      const uniqueSedes = Array.from(new Set(dataToExport.map(t => t.sede).filter(Boolean)));
      const sedeText = uniqueSedes.length === 1 ? String(uniqueSedes[0]).toUpperCase() : 'CONSOLIDADO GLOBAL';
      doc.text(`SEDE: ${sedeText}`, pageWidth / 2, 160, { align: 'center' });
      ...
      const dateText = isDaily 
        ? `DEL ${selectedDate.getDate().toString().padStart(2, '0')} DE ${monthNames[selectedDate.getMonth()]} DE ${selectedDate.getFullYear()}`
      ...
      doc.setFontSize(8);
      doc.setTextColor(30, 58, 138);
      doc.text(`${dataToExport.length} registros`, pageWidth / 2, 235, { align: 'center' });
      ...
      const tableData = sortedTxs.map(tx => {
        ...
        const timeStr = tx.hora || '';
        const dateCell = timeStr ? `${dateStr}\n${timeStr}` : dateStr;
      ...
      autoTable(doc, {
        startY: 270,
        head: [['Fecha / Hora', 'Sede', 'Caja / Cuenta', 'Tipo', 'Concepto / Detalle', 'Monto', 'Saldo']],
        body: tableData,
        theme: 'plain',
        headStyles: {
          fillColor: [30, 58, 138],
      ...
      doc.save(`Reporte_${isDaily ? 'DIARIO' : 'MENSUAL'}_Consolidado_Global_${todayStr}.pdf`);
  ...
  if (loading) {
    return (
      <div className="p-4 sm:p-6 w-full space-y-4 animate-pulse">
        {/* HEADER SKELETON */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
          <div className="space-y-2">
            <div className="h-8 w-48 bg-[var(--sidebar-bg)] dark:bg-slate-800 rounded-md"></div>
            <div className="h-4 w-64 bg-[var(--sidebar-bg)] dark:bg-slate-800 rounded-md"></div>
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="h-10 w-28 bg-[var(--sidebar-bg)] dark:bg-slate-800 rounded-lg"></div>
          </div>
        </div>

        {/* ROW 1: SKELETON */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-[#1a1f2e] border border-gray-100 dark:border-[var(--sidebar-border)] rounded-2xl p-6 flex flex-col justify-between min-h-[220px]">
            <div className="h-4 w-40 bg-slate-200 dark:bg-slate-700 rounded mb-6"></div>
            <div className="h-9 w-40 bg-slate-200 dark:bg-slate-700 rounded-lg mb-6"></div>
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="space-y-2">
                  <div className="flex justify-between">
                    <div className="h-3.5 w-24 bg-slate-200 dark:bg-slate-700 rounded"></div>
                    <div className="h-3.5 w-16 bg-slate-200 dark:bg-slate-700 rounded"></div>
                  </div>
                  <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-1.5"></div>
                </div>
              ))}
            </div>
          </div>
          <div className="bg-white dark:bg-[#1a1f2e] border border-gray-100 dark:border-[var(--sidebar-border)] rounded-2xl p-6 flex flex-col justify-between min-h-[220px]">
            <div className="flex justify-between">
              <div className="h-4 w-48 bg-slate-200 dark:bg-slate-700 rounded mb-6"></div>
              <div className="h-5 w-5 bg-slate-200 dark:bg-slate-700 rounded-full"></div>
            </div>
            <div className="mb-6">
              <div className="h-9 w-48 bg-slate-200 dark:bg-slate-700 rounded mb-2"></div>
              <div className="h-4 w-32 bg-slate-200 dark:bg-slate-700 rounded"></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="h-[72px] bg-slate-100 dark:bg-slate-800 rounded-lg"></div>
              <div className="h-[72px] bg-slate-100 dark:bg-slate-800 rounded-lg"></div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white dark:bg-[#1a1f2e] border border-gray-100 dark:border-[var(--sidebar-border)] rounded-2xl p-8 h-[460px]">
             <div className="flex justify-between items-center mb-8">
               <div className="h-4 w-32 bg-slate-200 dark:bg-slate-700 rounded"></div>
               <div className="h-6 w-24 bg-slate-200 dark:bg-slate-700 rounded"></div>
             </div>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-12 h-[360px]">
               <div className="h-full w-full bg-slate-100 dark:bg-slate-800 rounded-xl"></div>
               <div className="h-full w-full bg-slate-100 dark:bg-slate-800 rounded-xl hidden md:block"></div>
             </div>
          </div>
          <div className="bg-white dark:bg-[#1a1f2e] border border-gray-100 dark:border-[var(--sidebar-border)] rounded-2xl p-6 h-[460px]">
             <div className="h-4 w-40 bg-slate-200 dark:bg-slate-700 rounded mb-6"></div>
             <div className="h-[340px] w-full bg-slate-100 dark:bg-slate-800 rounded-xl"></div>
          </div>
        </div>

        {/* ROW 3: SKELETON */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white dark:bg-[#1a1f2e] border border-gray-100 dark:border-[var(--sidebar-border)] rounded-2xl p-6 min-h-[300px]">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               {[1, 2, 3].map(i => (
                 <div key={i} className="flex flex-col items-center">
                   ...
                   <div className="h-4 w-32 bg-slate-100 dark:bg-slate-800 rounded mt-4"></div>
                 </div>
               ))}
             </div>
          </div>
          <div className="bg-white dark:bg-[#1a1f2e] border border-gray-100 dark:border-[var(--sidebar-border)] rounded-2xl p-6 min-h-[300px]">
             <div className="h-4 w-32 bg-slate-200 dark:bg-slate-700 rounded mb-8"></div>
  ...
  const saldoTotal = distSedes.reduce((acc, c) => acc + (Number(c.value) || 0), 0);
  ...
  return (
    <div className="p-4 sm:p-6 w-full space-y-4 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
        <div>
          <h1 className="text-2xl font-black text-[var(--sidebar-text-hover)] tracking-tight">Panel de Operaciones</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-[#1a1f2e] border border-gray-100 dark:border-[var(--sidebar-border)] rounded-2xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-xs font-bold text-[var(--sidebar-text)] uppercase tracking-widest">Capital Total Consolidado</h3>
          ...
          {loadingSedes ? (
            ...
            <>
              ...
              <div className="space-y-4 max-h-[100px] overflow-y-auto pr-2 custom-scrollbar">
                {distSedes.map((sede, idx) => {
                  const porcentaje = saldoTotal > 0 ? ((sede.value / Number(saldoTotal)) * 100).toFixed(1) : 0;
                  return (
                    <div key={idx} className="space-y-1.5">
                      <div className="flex justify-between items-center text-xs font-bold text-[var(--sidebar-text-hover)]">
                        <span className="truncate max-w-[150px]">{sede.name}</span>
                        <div className="flex items-center gap-3">
                          <span className="text-[var(--sidebar-text)] font-semibold">S/ {Number(sede.value).toLocaleString('en-US', {minimumFractionDigits: 2})}</span>
                          <span>{porcentaje}%</span>
                        </div>
                      </div>
                      <div className="w-full bg-[var(--sidebar-bg)] rounded-full h-1.5">
                        <div className="h-1.5 rounded-full" style={{ width: `${porcentaje}%`, backgroundColor: sede.fill }}></div>
                      </div>
                    </div>
                  );
                })}
