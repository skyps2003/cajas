...
export const CajeroDashboard: React.FC = () => {
  ...
  return (
    <div className="p-4 sm:p-6 w-full space-y-4 animate-fade-in">
      ...
      {/* MODAL SUNAT ALERTS */}
      {showSunatModal && upcomingDeadlines.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#1a1f2e] w-full max-w-2xl rounded-2xl shadow-2xl border border-rose-100 dark:border-rose-900/50 overflow-hidden relative">
            <button 
              onClick={() => setShowSunatModal(false)}
              className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 p-1 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors z-10"
            >
              <X size={20} />
            </button>
