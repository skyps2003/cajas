...
import { X, Search, Plus, Edit2, Trash2, Key, CheckCircle2, XCircle } from 'lucide-react';
...
export const ContribuyentesTiposCredencialesPage: React.FC = () => {
  const { showToast } = useToast();
  const { user } = useAuth();
  const { isOpen, title, message, confirm, close, onConfirm } = useConfirm();
  
  const [tipos, setTipos] = useState<TipoCredencialResponse[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  
  // Modal states
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingRegistro, setEditingRegistro] = useState<TipoCredencialResponse | null>(null);
  ...
  const fetchTipos = async () => {
    ...
    try {
      setLoading(true);
      const data = await tipoCredencialService.getAll(user.token);
      setTipos(data);
    } catch (error) {
      console.error(error);
      showToast('error', 'Error', 'No se pudieron cargar los tipos de credenciales');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = (id: number, nombre: string) => {
    confirm(
      'Eliminar Tipo de Credencial',
      <>¿Está seguro de eliminar el tipo de credencial <strong>"{nombre}"</strong>?</>,
      async () => {
        if (!user?.token) return;
        try {
          await tipoCredencialService.delete(user.token, id);
          setTipos(prev => prev.filter(t => t.id !== id));
          showToast('success', 'Tipo Eliminado', `Se removió "${nombre}".`);
        } catch (error) {
          console.error(error);
          showToast('error', 'Error', 'No se pudo eliminar el tipo');
        }
      }
    );
  };
  ...
  const handleOpenEdit = (registro: TipoCredencialResponse) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!user?.token) return;
    if (!formData.nombre) {
      showToast('warning', 'Campos vacíos', 'Por favor complete el nombre del tipo de credencial.');
      return;
    }

    try {
      if (editingRegistro) {
        await tipoCredencialService.update(user.token, editingRegistro.id, formData);
        showToast('success', 'Tipo Actualizado', 'Se guardaron los cambios correctamente.');
      } else {
        await tipoCredencialService.create(user.token, formData);
        showToast('success', 'Tipo Creado', 'Se ha registrado el nuevo tipo.');
      }
      fetchTipos();
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-on-surface-variant mt-1 ml-3">
            Configuración y catálogo de tipos de credenciales para los contribuyentes.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-[#16212E] rounded-xl border border-slate-200 dark:border-[#1E2D3D] p-3 sm:p-4 shadow-sm flex items-center gap-3 sm:gap-4 hover:shadow-md transition-shadow">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="text-2xl sm:text-3xl font-black text-[#1B2E4B] dark:text-white tracking-tight leading-none">{tipos.filter(t => !t.estado).length.toString().padStart(2, '0')}</div>
          </div>
        </div>
      </div>
              ...
              {loading ? (
                ...
                filteredTipos.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50 dark:hover:bg-[#1E2D3D]/50 transition-colors">
                    <td className="px-6 py-4 font-semibold text-slate-800 dark:text-slate-100">{t.nombre}</td>
                ...
                <tr>
                  <td colSpan={3} className="text-center py-10 text-slate-400">
                    No se encontraron tipos registrados.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-[#16212E] w-full max-w-[400px] rounded-xl shadow-2xl relative flex flex-col border border-[#E2E8F0] dark:border-[#1E2D3D]" style={{ maxHeight: '90vh' }}>
            <button
            ...
            <div className="p-5 sm:p-6 overflow-y-auto no-scrollbar flex-1">
              <div className="mb-5">
                ...
                <h2 className="text-xl font-bold text-[#1B2E4B] dark:text-[#E8EDF5]">
                  {editingRegistro ? 'Editar Tipo' : 'Agregar Tipo'}
                </h2>
              </div>
            
              <form id="tipo-credencial-form" onSubmit={handleSave} className="space-y-4">
                <div className="flex flex-col">
                  <label className={labelCls}>Nombre del Tipo</label>
                  <input 
                    type="text" 
                    placeholder="Ej: EXTRANET - SUNARP"
                    required
                    className={inputCls}
            ...
            <div className="px-5 py-3.5 bg-[#F5F6FA] dark:bg-[#0D1825] rounded-b-xl flex items-center justify-center gap-3 shrink-0 border-t border-[#E2E8F0] dark:border-[#1E2D3D]">
              <button type="submit" form="tipo-credencial-form" className="px-6 py-2.5 bg-[#C4933F] hover:bg-[#A87A30] text-white rounded-lg text-sm font-bold transition-colors shadow-sm min-w-[160px] flex items-center justify-center gap-2">
                GUARDAR
              </button>
