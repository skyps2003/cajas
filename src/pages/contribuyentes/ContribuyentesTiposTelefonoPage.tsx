...
import { X, Search, Plus, Edit2, Trash2, Smartphone, CheckCircle2, XCircle } from 'lucide-react';
...
export const ContribuyentesTiposTelefonoPage: React.FC = () => {
  const { showToast } = useToast();
  const { user } = useAuth();
  const { isOpen, title, message, confirm, close, onConfirm } = useConfirm();
  
  const [tipos, setTipos] = useState<TipoTelefonoResponse[]>([]);
  const [loading, setLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState('');
  
  // Modal states
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingRegistro, setEditingRegistro] = useState<TipoTelefonoResponse | null>(null);
  ...
  const fetchTipos = async () => {
    ...
    try {
      setLoading(true);
      const data = await tipoTelefonoService.getAll(user.token);
      setTipos(data);
    } catch (error) {
      console.error(error);
      showToast('error', 'Error', 'No se pudieron cargar los tipos de teléfono');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = (id: number, nombre: string) => {
    confirm(
      ...
      <>¿Está seguro de eliminar el tipo de teléfono <strong>"{nombre}"</strong>?</>,
      async () => {
        if (!user?.token) return;
        try {
          await tipoTelefonoService.delete(user.token, id);
  ...
  const handleOpenEdit = (registro: TipoTelefonoResponse) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!user?.token) return;
    if (!formData.nombre) {
      showToast('warning', 'Campos vacíos', 'Por favor complete el nombre del tipo de teléfono.');
      return;
    }

    try {
      if (editingRegistro) {
        await tipoTelefonoService.update(user.token, editingRegistro.id, formData);
        showToast('success', 'Tipo Actualizado', 'Se guardaron los cambios correctamente.');
      } else {
        await tipoTelefonoService.create(user.token, formData);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-on-surface-variant mt-1 ml-3">
            Configuración y catálogo de tipos de teléfono para los contribuyentes.
          </p>
        </div>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-[#16212E] w-full max-w-[400px] rounded-xl shadow-2xl relative flex flex-col border border-[#E2E8F0] dark:border-[#1E2D3D]" style={{ maxHeight: '90vh' }}>
            ...
            <div className="p-5 sm:p-6 overflow-y-auto no-scrollbar flex-1">
              ...
              <form id="tipo-telefono-form" onSubmit={handleSave} className="space-y-4">
                <div className="flex flex-col">
                  <label className={labelCls}>Nombre del Tipo</label>
                  <input 
                    type="text" 
                    placeholder="Ej: Fijo, Móvil, Trabajo"
                    required
                    className={inputCls}
            ...
            <div className="px-5 py-3.5 bg-[#F5F6FA] dark:bg-[#0D1825] rounded-b-xl flex items-center justify-center gap-3 shrink-0 border-t border-[#E2E8F0] dark:border-[#1E2D3D]">
              <button type="submit" form="tipo-telefono-form" className="px-6 py-2.5 bg-[#C4933F] hover:bg-[#A87A30] text-white rounded-lg text-sm font-bold transition-colors shadow-sm min-w-[160px] flex items-center justify-center gap-2">
                GUARDAR
              </button>
